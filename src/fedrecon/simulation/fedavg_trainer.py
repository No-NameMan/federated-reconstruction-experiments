...
def _parse_max_final_eval_clients(config: dict) -> int | None:
...
def run_fedavg_global_training(config: dict) -> Path:
    ...
    train_client_ids = sorted(data.train_clients.keys())
    ...
    eval_reconstruction_steps = int(
        config["evaluation"].get(
            ...
            config.get("reconstruction", {}).get("steps", 10),
        )
    )
    ...
    eval_reconstruction_lr = float(
        config["evaluation"].get(
            "reconstruction_lr",
            config.get("reconstruction", {}).get("lr", 0.1),
        )
    )
    ...
    for round_idx in trange(1, rounds + 1, desc="FedAvg-like rounds"):
        ...
        result = run_fedavg_global_round(
            model=model,
            selected_clients=selected_clients,
            local_steps=int(config["fedavg"]["local_steps"]),
            local_lr=float(config["fedavg"]["lr"]),
    ...
    max_final_eval_clients = _parse_max_final_eval_clients(config)
